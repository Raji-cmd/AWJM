...
def create_awjm_architecture_diagram():
    ...
    # ===== Helper: Draw Box =====
    def draw_box(x, y, text, fc, fontsize=10, bold=False):
    ...
    # --- Temporarily increase box size for Ensemble ---
    orig_w, orig_h = box_w, box_h
    box_w, box_h = 3.2, 1.5  # enlarged box
    ...
    # ==== Legend ====
    legend_elements = [
        Rectangle((0, 0), 1, 1, fc=input_color,    label='Input Parameters'),
        Rectangle((0, 0), 1, 1, fc='#C1FFD7',      label='Regression Models'),
        Rectangle((0, 0), 1, 1, fc=ensemble_color, label='Ensemble Layer'),
        Rectangle((0, 0), 1, 1, fc=output_color,   label='Output Metrics')
    ]
