...
def plot_learning_curve(data, title, ylabel):
    # X = predictions from different models
    X = np.column_stack([data['SVR'], data['ElasticNet'], data['XGBoost'], data['NeuralNet']])
    y = np.array(data['Exp.value'])

    svr = SVR()
    enet = ElasticNet()
    xgb_like = Ridge()  # Placeholder for XGBoost in VotingRegressor
    nn = MLPRegressor(max_iter=1000, random_state=0)

    ensemble = VotingRegressor([('svr', svr), ('enet', enet), ('xgb', xgb_like), ('nn', nn)])
