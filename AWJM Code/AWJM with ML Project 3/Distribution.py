...
colors = {
    'surface_roughness_um': ('#E0BBE4', '#6A0DAD'),      # Light Purple → Dark Purple
    'material_removal_rate_mm3_min': ('#A8E6CF', '#008000'),  # Mint Green → Dark Green
    'kerf_angle_deg': ('#B0E0E6', '#00008B')             # Powder Blue → Dark Blue
}
...
for i, target in enumerate(targets):
    ...
    axes[i].hist(train, bins=8, alpha=0.7, color=colors[target][0], edgecolor='black', label='Train')
    axes[i].hist(test, bins=8, alpha=0.7, color=colors[target][1], edgecolor='black', label='Test')
