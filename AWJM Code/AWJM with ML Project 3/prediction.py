...
def get_predictions(X, y, test_size=0.2):
    ...
    for name, model in models.items():
        ...
        if name in ['Neural Network', 'SVR', 'ElasticNet']:
...
# Function to calculate metrics (R², RMSE, MAE)
...
for target_name, y in targets.items():
    ...
    for i, (name, pred) in enumerate(predictions.items()):
        plt.plot(exp_nos, pred, 's-', color=colors[i], markersize=6, label=name, alpha=0.9, linewidth=2)
    ...
    # Legend outside
    plt.legend(loc='upper left', bbox_to_anchor=(1.05, 1), fontsize=10, markerscale=1, frameon=True, borderpad=0.3)
