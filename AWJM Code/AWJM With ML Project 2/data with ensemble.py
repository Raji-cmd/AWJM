...
def run_models(y):
    ...
    # === Weighted Ensemble (for all 6 models) ===
    weights = {
        "Ridge": 0.10,
        "RandomForest": 0.15,
        "SVR": 0.20,
    ...
    ensemble_pred = sum(preds[m] * w for m, w in weights.items())
    preds["Weighted_Ensemble"] = ensemble_pred
...
with pd.ExcelWriter("Data_All6Models_Weighted_Ensemble.xlsx") as writer:
...
print("✅ Saved: Data_All6Models_Weighted_Ensemble.xlsx (Includes R², RMSE, MAE + Weighted Ensemble for all models)")
