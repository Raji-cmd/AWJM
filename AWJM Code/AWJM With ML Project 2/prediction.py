...
# Weights for ensemble: Ridge, RF, SVR, ElasticNet
...
def get_predictions(X, y, test_size=0.2):
    ...
    for name, model in models.items():
        # Standardize features for SVR and ElasticNet
        if name in ['SVR', 'ElasticNet', 'Ridge']:
        ...
        # Full dataset prediction (train + test)
    ...
    for w, model_name in zip(weights, ['Ridge', 'Random Forest', 'SVR', 'ElasticNet']):
        weighted_pred += w * predictions[model_name]
    predictions['Weighted Ensemble'] = weighted_pred
    scores['Weighted Ensemble'] = r2_score(y, weighted_pred)
...
for target_name, y in targets.items():
    ...
    plt.plot(exp_nos, y, 'ko-', linewidth=3, markersize=8, label='Experimental', markerfacecolor='black')
    ...
    plt.xlabel('Experiment Number', fontsize=16, fontweight='bold')
    plt.ylabel(target_name, fontsize=16, fontweight='bold')
